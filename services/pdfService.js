...
async function buildPDF(dataCallback, endCallback, data,examen) {
  ...
  pdf.setDocumentHeader({
      ...
      pdf.moveDown();
      pdf.font('Helvetica-Bold').fontSize(10).fillColor('#8C2E40').text('Facultad de ciencias naturales ').moveDown(0.5);
      pdf.font('Helvetica-Bold').fontSize(18).fillColor('black').text(`${examen.fechaCreacion} ${new Date().getFullYear()}`);
      pdf.font('Helvetica').fontSize(10).fillColor('black').text('Laboratorio de patologóa veterinaria');
      pdf.moveDown();
      pdf.font('Helvetica-Bold').fontSize(18).fillColor('#8C2E40').text(examen.fechaCreacion);
      pdf.moveDown();
  ...
  pdf.addTable(
    ...
    [
      {"numberRow": "ENTRENADOR(A)", "apellidos": examen.fechaCreacion, "nombres": examen.fechaCreacion,},  //Aqui es donde se debe colocar el nombre y apellido del entrenador para que se muestren en la tabla.
      {"numberRow": "ASISTENTE", "apellidos": examen.fechaCreacion, "nombres":examen.fechaCreacion,} //Aqui es donde se debe colocar el nombre y apellido del asistente para que se muestren en la tabla.
    ], {
        width: "fill_body",
        border : {size: 0.1, color: '#707475'},
        // cellsPadding: 6,
